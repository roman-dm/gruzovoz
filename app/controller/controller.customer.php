<?php
	Class admin_customer extends base{
		...
		public function main(){
			...
			echo $this->view->render('customer_main.html',array('profile'=>$this->profile[0],"news_mass"=>$news_mass,"user_list"=>$this->user_mas));
		...
		public function tasks(){
			...
			$tasks["all"]=$this->modelTask->getTaskAllMonth($_SESSION["id"],"all");
			$tasks["actual"]=$this->modelTask->getTaskActual($_SESSION["id"],"all");
			$tasks["success"]=$this->modelTask->getTaskSuccesed($_SESSION["id"],"all");
			$tasks["nosuccess"]=$this->modelTask->getTaskNoSuccesed($_SESSION["id"],"all");
		...
		public function events(){
			...
			$events["all"]=$this->model->getEventAllMonth($_SESSION["id"],"all");
			$events["actual"]=$this->model->getEventActual($_SESSION["id"],"all");
			$events["noactual"]=$this->model->getEventNoActual($_SESSION["id"],"all");
		...
		public function monitoring(){
			...
			$list["all"]=$this->model->getMonitoring();
			$class_list=array("6","7","8","9","10");
			foreach($class_list as $class_element){
				$list["monitoring_".$class_element."class"]=$this->model->getMonitoringClass($class_element);
			}
			echo $this->view->render('monitoring.html',array('profile'=>$this->profile[0],"monitoring"=>$list));
		}
	}